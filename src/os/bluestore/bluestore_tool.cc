...
int main(int argc, char **argv)
{
  ...
  if (action == "fsck" || action == "repair" || action == "quick-fix" || action == "allocmap" || action == "qfsck" || action == "restore_cfb" || action == "downgrade-wal-v1") {
    if (path.empty()) {
  ...
  if (action == "restore_cfb") {
    ...
    } else if (action == "downgrade-wal-v1") {
